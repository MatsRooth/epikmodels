...
nature and complexity as the world we occupy \citep{lewis1986plurality}.  But
...
application \citep{kozen2001automata}.\marginpar{2001 or 2003?}  The
models we propose are an epistemic extension of guarded string models
for KAT, where a unary operation interpreted as an existential
epistemic modality is included for each agent.  {\em Action models} in
dynamic epistemic semantics introduced the technique of constructing
epistemic models from primitive alternative relations on events, in
order to capture the epistemic consequences of perceptual events
\citep{baltag1999logic}. This is the basis for our construction of
epistemic alternative relations.  Literature on finite state methods
in linguistic semantics has used event strings and sets of event
strings to theorize about tense and aspect in natural language
semantics \citep{fernando2004finite,fernando2007observing,carlson2009tense} and to
express intensionality \cite{fernando2017intensions}.  Literature on
finite state intensiononal semantics has discussed how to do the
semantics of intensional complementation including indirect questions
in a setting where compositional semantics is expressed in a finite
state calculus \citep{rooth2017finite,collard2018finite}.  We adopt
this in our syntax-semantics interface for English.
...
of $a_1$, and $\overline{pq}$ being the precondition of $u$. Juxtaposition is
used for Boolean conjunction, and the overbar for
...
In this discussion, a sequence such as $\bar{q}{p}$ can be viewed as a
valuation of the primitive propositions $q$ and $p$, which is used to
describe world state. The primitives are listed in fixed order, and
left unmarked (indicating true) or marked with the overbar (indicating
false).  Suppose that in the coin example, we had an additional
primitive stative proposition (or atomic test) $t$, interpreted as
tails.  Since a coin is heads or tails but not both, we want to allow
the valuations $h\bar{t}$ and $\bar{h}t$, and disallow $ht$ and
$\bar{h}\bar{t}$.  This is enforced by a {\em state formula},
which is a simply Boolean formula, in this case \exi{effect}{a}.
Where
{\sf{B}} is a set of atomic tests and $\rho$ is a state constraint
over {\sf{B}}, ${\cal{A}}_{\mbox{\sf\tiny{B}}}^{\rho}$ is the set of valuations of
{\sf{B}} that make formula $\rho$ true. 

Formulas like the ones in \ex{event1} that describe pre- and
post-conditions are {\em effect formulas}. They are interpreted as
defining relations between valuations, as defined in Figure
\ref{effectfigure}.  The valuations they relate are constrained by the
...
specified in \ex{effect}. Then the set of valuations determined by
$\rho$ and the relation on valuations for the event $u$ are
$\sem{\rho}$ and $\sem{\zeta}^{\sem{\rho}}$ as given at the bottom in
\ex{6}.  Given a state formula $\rho$, the the relation characterizing
pre- and post-conditions of an event with effect formula $\zeta$ is
defined to be $\sem{\zeta}^{\sem{\rho}}$.

\begin{example}   \label{effect}
\begin{tabular}[t]{l}
State formula ($\rho$)  $h\bar{t} + \bar{h}t$ \\
Effect formula for $a_1$ ($\zeta$)  $\ev{h}{h}$ \\
\( \sem{\rho} = \{h\bar{t},\bar{h}t\} \)  \\
\( \sem{\zeta}^{\sem{\rho}} = \{\pair{h\bar{t}}{h\bar{t}}\} \)  \\
\end{tabular}
\end{example}  


\begin{figure}
\hspace{-2mm}\begin{tabular}{lll}
  state formulas & $\rho,\sigma...$ & $\rho{:}{:}{=} \, \begin{array}[t]{l}\tuple{\mbox{atomic tests}}\,|\\
    0\,|\,1\,| \; \rho{+}\sigma\;| \\ \rho\,{\&}\,\sigma\;|\;\bar{\rho}\end{array}$ \\
  effect formulas & $\zeta,\eta...$ & $\zeta{:}{:}{=} \, \begin{array}[t]{l} \rho\,{:}\,\sigma \,|\, \zeta{+}\eta \\
    \zeta\,{\&}\,\eta\;|\;\bar{\zeta}\end{array}$
\end{tabular}
\vspace{1mm}

\(
\begin{array}{cll}
  \sem{\rho} & = & \mbox{Set of Boolean valuations of} \\
            && \mbox{the atomic tests that satisfy $\rho$.} \\
\sem{\rho\,{:}\,\sigma}^V & = & (\sem{\rho}\cap V)\times(\sem{\sigma}\cap V)\\
\sem{\zeta{+}\eta}^V & = & \sem{\zeta}^V\cup\sem{\eta}^V\\
\sem{\zeta{\&}\eta}^V & = & \sem{\zeta}^V\cap\sem{\eta}^V\\
\sem{\bar{\zeta}}^V & = & V \times V \setminus \sem{\zeta}^V\\
\end{array}  
\)

\caption{Syntax and semantics of state formulas and effect formulas. Effect formulas
  are evaluated relative to as set Boolean valuations $V$, and denote relations between
  Boolean valuations.}
\label{effectfigure}
\end{figure}
...
Figure \ref{epikfigure} shows an Epik program that describes a
possible worlds model for two agents with information about one coin,
and events of the agents semi-privately looking at the coin. The line
beginning with {\tt state} lists the basic stative propositions. To
illustrate syntax, a second proposition $t$ (tails) is included.  The
line beginning with {\tt constraint} defines compatibilities among the
propositions: the coin is heads or tails and not both.  The lines
beginning with {\tt event} declare events, their preconditions, and
their effect on state, following the format in \ex{event1}.  Finally
the lines beginning with {\tt agent} define {\em event alternative}
relations for agents.  Each clause with an arrow has a single event
symbol on the left, and a disjunction of alternative events on the
right of the arrow. The interpretation of Amy's alternatives for $b_1$
(Bill peeks at heads), is that when $b_1$ happens, for Amy either
$b_1$ or $b_0$ (Bill peeks at tails) could be happening.


Kleene Algebra with Tests is an algebraic theory that is defined by
equations and inequalities, which a has model classes including
guarded string models, relational models, finite models, and matrix
models.  This paper focuses on defining a family of concrete guarded
string algebras, the elements of which are sets of guarded strings.
Definitions and notation mostly follow \cite{kozen2001automata}.
Additional syntax and semantics is included to model multi-agent
epistemic semantics.  Guarded strings over a finite alphabet {\sf{P}}
are like ordinary strings, but with truth assignments to a set $T$ of
primitive propositions (primitive tests) alternating with the symbols
from $E$. Following \cite{kozen2001automata}, we call these truth
assignments \emph{atoms}; the set of atoms is notated $\mathcal A$.
In the algebra described by Figure \ref{epikfigure}, $E$ is the set of
events $\{a_1,a_0,b_1,b_0\}$, and in the elevator example, $\{u,d\}$.
In the elevator example, $T$ is $\{p,q\}$, and in the coin example it
is $\{h,t\}$.  We write atoms as the boolean formulae that uniquely
specify the truth assignment. In the elevator example, we have atoms
$\bar q \bar p$, $\bar q p$, $q \bar p$, $q p$, for notational
simplicity we will write these as $\hat0$, $\hat1$, $\hat2$, $\hat3$
respectivelyto indicate the floor each atom represents. In the coin
example, we get $h\bar t$ and $\bar h t$ (for which we use the
shorthand $H$ and $T$), as well as the bogus atoms $\bar h \bar t$ and
$ht$!  To preclude bogosity, the \texttt{constraint} declaration in
Figure~\ref{epikfigure} describes the set of \emph{feasible atoms},
which we parse into a formula $\varphi$ (if there are multiple
\texttt{constraint} declarations, $\varphi$ is the conjunction of the
parsed formulae in each). For a formula $\varphi$, the set of atoms
that satisfy $\varphi$ is $\mathcal A_\varphi$ -- these are the
feasible atoms. In the coin example, the constraint $\varphi$ is
$h\bar t + \bar h t$. Since $\bar h t$ satisfies $\varphi$ and $h \bar
t$ satisfies $\varphi$, they are permitted feasible atoms, and since
$\bar h \bar t$ and $h t$ do not satisfy $\varphi$ they are not
permitted. We omit boolean satisfiability since its definition is
standard.

Guarded strings are strings of events, alternating with such atoms,
and starting and ending with atoms.  \ex{gs1} gives the encoding as
guarded strings of the worlds in \ex{elevator1} and \ex{coin1}.

%% \begin{example}   \label{gs1}
%% \begin{tabular}[t]{ll}
%% $v_1$ & $\bar p \bar q ~u~ \bar p q ~u~ q \bar p ~d~ \bar p q ~u~ q \bar p$ \\
%% $v_2$ & $\bar p \bar q ~u~ \bar p q ~u~ p\bar q ~u~ pq$ \\   
%% $w_1$ & $\bar h t$ \\
%% $w_2$ & $\bar h t ~a_0~ \bar h t$ \\  
%% $w_3$ & $\bar h t ~a_0~ \bar h t ~b_0~ \bar h t$ \\
%% \end{tabular}
%% \end{example}
\begin{example}   \label{gs1}
\begin{tabular}[t]{ll}
$v_1$ & $\hat 0 ~u~ \hat 1 ~u~ \hat 2 ~d~ \hat 1 ~u~ \hat 2$ \\
$v_2$ & $\hat 0 ~u~ \hat 1 ~u~ \hat 2 ~u~ \hat 3$ \\   
$w_1$ & $H$ \\
$w_2$ & $H ~a_0~ H$ \\  
$w_3$ & $T~a_0~ T ~b_0~ T$ \\
\end{tabular}
...
\hspace{-2mm}\( \hat 0 ~u~ \hat 1 ~u~ \hat 2 ~d~ \hat 1 \diamond \hat 2 ~u~ \hat 3 = \mbox{\em{undefined}} \)

\hspace{-2mm}\(H \diamond H ~a_0~ H = H ~a_0~ H \)

\hspace{-2mm}\(T \diamond T ~a_1~ T = \mbox{\em{undefined}} \)
\end{example}

Rather than individual guarded strings, elements of a guarded string
model for KAT are sets of guarded strings.  In the application, these
elements have the interpretation of propositions (sets of possible
worlds) and/or event types.  An event such as $u$ in the guarded
string model corresponds to the set of guarded strings where the bare
event is flanked by compatible atoms. In standard KATs, every possible
atom is allowed to surround every event, which leads to nonsensical
strings like $\hat 1 ~u~\hat 0$ in the denotation of $u$, indicating
that the elevator travelled up from floor 1 to floor 0 (a physical
impossibility).

Fortunately, Epik provides a syntax to specify the input-output
behavior via the \texttt{event} declarations. From these we construct
a function $P$ from agents to relations on atoms that specify the
effects each action can have. If there are multiple event declarations
for the same event, only the latter one is incorporated. To construct
the effect relation $P(e)$ for event $e$, we add a pair $\alpha,
\beta$ to the relation $P(e)$ for every disjunct $\varphi:\varphi'$ such
that $\alpha$ satisfies $\phi$ and $\beta$ satisfies $\varphi'$.  These have the
status of event types, in that they can ``happen'' in different
possible worlds. Here happening corresponds to incrementing the world
with some compatible element of the event type.  Where $e$ is a bare
event symbol, $\llbracket e \rrbracket^{\varphi,P}$ is the
corresponding set of guarded strings, consisting of the bare event
decorated with compatible truth assignments denoted by $P$ that are
admitted by the world-constraints $\varphi$.  See this formally in
Figure~\ref{fig:interp}, and the examples in \ex{events1}.
% u (\bar{q}\bar{p}):(\bar{q}p) + (\bar{q}p):(q\bar{p}) + (q\bar{p}):(qp) 173
...
\(
\begin{array}[t]{l|c}
  e & \llbracket e \rrbracket^{\varphi, P} \\ \hline
...
\section{A Language for Epistemic Sets of Guarded Strings}

To reason about epistemic alternatives, guarded strings are used to
represent different possible worlds, so agents' knowledge/beliefs are
represented by sets of guarded strings. The standard algebra for
manipulating guarded strings is Kleene Algebra with Tests
(\textsc{KAT}), which has the algebraic signature
$\tuple{K,+,;,*,\bar{},0,1}$ \cite{kozen2001automata}. However, the
KATs don't permit native reasoning about modal operators, so Epik's
signature needs to subsume the KAT operators.

Figure~\ref{fig:epik-grammar} depicts the syntax of Epik's formal
term language. As in KAT, $0$ is the null element, $1$ is the unit
element, $+$ indicates disjuction, $;$ indicates event sequencing or
boolean conjunction, $!$ indicates boolean negation, and $*$ indicates
iteration. We add some additional operations to reason about epistemic
alternatives: the term $\Diamond_a~p$ represents epistemic possibility
for agent $a$ in worlds $p$, and complement $\neg p$ represents the
worlds not described by $p$. We can encode $\Box_a~p$, i.e. epistemic
certainty for agent $a$ in worlds $p$, by $\Box_a~p =
(\Diamond_a~p^c)^c$.

\begin{figure}

  \[\begin{array}{l>{\hspace{-.5em}}c>{\hspace{-0.75em}}l}
  \multicolumn 2 l {\textrm{tests}}& a~\textrm{is an atomic test} \\
  t,s & ::=& a~|~0~|~1~|~t+t~|~t;t~|~!t\\
  \multicolumn 2 l {\textrm{terms}} & e~\textrm{is an atomic event} \\
  p,q & ::=& e~|~0~|~1~|~t~|~p+p~|~p;p~|~p^*~|\neg p~|\Diamond_a~p
  \end{array}\]

  Derived Operators\\
  \mbox{}\qquad \(\Box_a p \triangleq \neg\Diamond_a\neg p \qquad p\&q \triangleq \neg(\neg p + \neg q) \)
...
We interpret each term in as a set of guarded strings as indicated in
Figure~\ref{fig:interp}. The function $\llbracket p
\rrbracket^{\varphi,P}$ interprets a term $p$ as a set of guarded
strings consistent with state constraints $\varphi$ and effect
relation $P$. A bare event $e$ is interpreted as the set of guarded
strings allowed by $\varphi$ and $P$. The constant $0$ is interpreted
as the empty set.  The constant $1$ is interpreted as the set of all
atoms obeying $\varphi$, i.e. $\{h\bar t, \bar h t\}$ in the coin
example. The operation $p + q$ is interpreted as the union of the
interpretations of $p$ and $q$. The operation $p;q$ interpreted as the
fusion product of the interpretations of $p$ and $q$ raised to sets:
$X \diamond Y = \{ x \diamond y \mid x \in X, y \in Y, x\diamond
y~\textit{defined}\}$.  The operation $p*$ is Kleene star, which is
interpreted to be least upper bound of the iterated fusion product of
the denotation of $p$ with itself. Subsets of $1$ indicated by $t$ are
also elements of $K$, and these form the Boolean algebra of tests,
denoting all atoms admitted by the constraint $\varphi$ and the
boolean test $t$.  We ``overload'' $;$ to further indicate boolean
conjunction, $+$ to further indicate boolean disjunction, and $0,1$ to
indicate the zero and one elements of the boolean algebra. The overbar
operation denotes complement in this Boolean algebra: $\overline t$
denotes all atoms $\alpha$ such that $\alpha \Rightarrow \neg t$.
Other boolean operators (such as implication $\Rightarrow$) are
encoded as syntactic sugar in the standard way. A boolean test $t$
denotes the set of atoms that satisfy $t;\varphi$, that is, they
satisfy both $t$ and $\varphi$.

The unary epistemic alternative operation $\Diamond_a$ for each agent
$a$, is interpreted using using Kripke semantics, as pre-image
relative to a fixed relation $\hat{R_a}$ between guarded strings,
$\Diamond_a~x = \{u \mid \exists v. v \in x \wedge u \mathrel{R_a}
v\}$.  Here $u$ and $v$ are guarded strings, while $x$ is an element
of $K$.
...
It remains to define the Kripke relation on guarded strings from an
agent specification as in Figure \ref{epikfigure}. In Epik, an agent
specification pairs each bare event with a set of ($+$-separated) bare
events, and so determines a relation between bare events, call it
relation $R_a$ for an agent $a$.  We can lift this to a relation
$\hat{R_a}$ between guarded strings, as defined in \ex{Rhathat}. The
operation $(\mathop;)$ in the definition is KAT product, which enforces
matching of tests.

%% \begin{example}   \label{Rhat}
%% \hspace{-2mm} \(
%% \hat{R} \triangleq
%% \{\tuple{u,v} \mid \exists c \exists d. c\mathrel R d \wedge u \in \hat{c}  \wedge v \in \hat{d} \, \} \)
%% \end{example}
...
\{ \tuple{x,y} \mid \exists u_1 ... \exists u_n \exists v_1 ... \exists v_n. \\
\hspace{6mm} u_1\mathrel{R}v_1 \wedge ... \wedge u_n\mathrel{R}v_n \wedge\\
\hspace{6mm} x \in \llbracket u_1 ; ... ; u_n \rrbracket^{\varphi,P}\hspace{1mm} \wedge \\
\hspace{6mm} y \in \llbracket v_1 ; ... ; v_n \rrbracket^{\varphi,P} \hspace{2mm} \}
\end{array}
\)
\end{example}
...
\begin{figure}
  ...
  \llbracket 0 \rrbracket^{\varphi,P} && \{\} \\  
  \llbracket 1 \rrbracket^{\varphi,P} && \{\alpha \mid \alpha \in \mathcal A_\varphi\}\\
  \llbracket e \rrbracket^{\varphi,P}&& \{\alpha~e~\beta \mid
  (\alpha,\beta) \in P(e), \alpha, \beta \Rightarrow \varphi \}
  \\ \llbracket \_ \rrbracket^{\varphi,P}&& \bigcup_{e \in
    \dom(P)}\llbracket e \rrbracket^{\varphi,P}\\
  \llbracket t \rrbracket^{\varphi,P} &&  \{\alpha \mid \alpha \in \mathcal A_{\varphi;t}\}\\
  \llbracket{p+q}\rrbracket^{\varphi,P} && \llbracket{p}\rrbracket^{\varphi,P} \cup \llbracket q \rrbracket^{\varphi,P}\\
  \llbracket{p;q}\rrbracket^{\varphi,P} && \llbracket p \rrbracket^{\varphi,P} \diamond \llbracket{q}\rrbracket^{\varphi,P}\\
  \llbracket p^*\rrbracket^{\varphi,P} && \displaystyle\bigcup_n {\underbrace{\llbracket p \rrbracket^{\varphi,P}\diamond \ldots \diamond \llbracket p \rrbracket^{\varphi,P}}_{n}} \\
  \llbracket \neg p \rrbracket^{\varphi,P} && \llbracket \_^*\rrbracket^{\varphi,P} \setminus \llbracket p \rrbracket^{\varphi,P} \\
  \llbracket \langle a \rangle~p \rrbracket && \{x \mid x~\hat{R_a}~y, y \in \llbracket p \rrbracket\}
...
The complement operation ${p}^c$ is complement at the level of sets of
guarded strings, defined to every set the universe $\_^*$ except those
denoted by $p$. Note that $\llbracket 0^c \rrbracket^{\varphi,P} =
\llbracket\_^*\rrbracket^{\varphi,P}$. We use the complement to define
universal box modalites as the dual of the diamond modality, $\Box_a x
= (\Diamond_a x^c)^c$.  For instance, Aly is certain that the coin is
heads if and only if she does not consider it possible that it is not
heads.

Summing up, given an Epik program with $n$ agents, we construct state
constraints $\varphi$, effect relations $P$, and a concrete guarded
string model for each term using $\llbracket -
\rrbracket^{\varphi,P}$.
%% $\tuple{K,+,;,*,\bar{},0,1,{}^c,\Diamond_{a_1},...,\Diamond_{a_n}}$.
 $0^c$ is the set of worlds, and it may be countably infinite.
$\Diamond_{a_i}$ is an epistemic modality for the $i$th agent.  Or
referring to the Kripke relations $\hat{R}_i$, the construction
defines a multi-agent Kripke frame
$\tuple{0^c,\hat{R}_1,...,\hat{R}_n}$ (usually a countable one) from
an Epik specification. The frame consists of a set of worlds, and an
epistemic-alternative relation for each agent.  These models are used
as a target for natural-language interpretation in Section 5 and
Section 6, where we obtain semantic values such as \sem{Amy knows that
  it's heads and Bob knows that Amy knows whether it is heads or
  tails, and does not know that it's heads} as elements of
$K$. Concretely the proposositions are sets of guarded strings
(usually countable ones), construed as sets of worlds as they figure
in possible worlds semantics for natural language.
...
morphographemics 
\citep{kaplan1994regular,beesley2003finite}.
Current implementations allow for the
definition of functions with the status of defined operators on regular sets and
relations
\citep{hulden2009foma,karttunen2010update}.
Such definitions are used to define an embedding of epistemic KAT in a
...
Kleene star are defined operations in the string algebra, see Figure \ref{fstfigure1}.  The
...
This scheme provides for an interpretation of the language of Epik
terms that is defined in Figure \ref{termfigure}.  An Epik specifications such as
Figure 1 is translated to a straight-line program of the finite state
calculus that defines constants and functions, including the ones from
Figures 2 and 3.  In Xfst or Foma, which are interpreters for the
finite state calculus, the program is read, and then propositional
terms can be mapped to finite state machines that represent sets of
guarded strings, interpreted as propositions.


\section{Direct Bounded Interpretation}

We also implement the semantics of Epik terms using lazy lists~\cite{LazyLists}
in Haskell, rather than the direct interpretation as sets. Unfortunately,
regular expressions, and hence also Epik, can denote infinite sets of strings,
for example the term $\_^*$. Normally regular languages are represented using a
finite coalgebra~\cite{}. However the non-distributivity of $\diamond$ accross
$;$ complicates this construction~\footnote{Axiomatic and Coalgebraic models for
  Epik are open questions}. To sidestep this, we parameterize the interpretation
function on a positive integer $n$ and only produce guarded strings of length
$n$ or less.  The length of a guarded string $g$, written $|g|$ is the number of
events in the string, so $\alpha$ has length $0$, $\alpha e\beta$ has length
$1$, and $\alpha e \beta e' \gamma$ has length $2$.

We translate the Epik terms into a Haskell algebraic datatype that represents
terms with the same signature as described in Section 2. We then paramaterize
the interpretation function, $\llparenthesis p \rrparenthesis^{\varphi,P}_n$ on an
integer $n$ that describes the maximum length of a string we will produce. The
bounded interpretation into lists of strings is very similar to the unbounded
interpretation into sets of strings, except for the bounds checking. The full
details are shown in Figure~\ref{fig:list-interp}: we use list-comprehension
notation, which is analogous to set-builder notation, except that it produces a
list (potentially with duplicated elements) instead of a set, and so is written
with square brackets $[]$. We use $+$ to indicate list concatenation, and $\sum
l$ to indicate the concatenation of a list of lists $l$. We overload the set
membership symbol $\in$ to also indicate membership in a list. We also lift the
fusion operator $\cdot$ to lists. For a list of guarded strings $l$, we write
$l|_n$ as shorthand for the set $[ g~|~g \in l , |g| \leq n]$.

First of all, whenever $n = 0$, the set of guarded strings is empty. TODO ADD
MORE TEXT.

The final step is to convert the list of guarded strings into a set of guarded
strings. Since a list is just an ordered multiset, we forget the order, and
remove all duplicates. Haskell's libraries have a built-in function for this,
\texttt{Data.Set.fromList}, which converts a list into a set.

\begin{figure}
  \[\begin{array}{r >{\triangleq}c l}
  \llparenthesis p \rrparenthesis^{\varphi,P}_0 && [] \\
  \llparenthesis 0 \rrparenthesis^{\varphi,P}_n && [] \\
  \llparenthesis 1 \rrparenthesis^{\varphi,P}_n && [\alpha \mid \alpha \leq \varphi] \\
  \llparenthesis e \rrparenthesis^{\varphi,P}_n && [\alpha \cdot e \cdot \beta \mid (\alpha, \beta) \in P(e), \alpha\vee \beta \Rightarrow \varphi] \\
  \llparenthesis \_ \rrparenthesis^{\varphi,P}_n && \sum [\llparenthesis e \rrparenthesis^{\varphi,P} \mid e \in \dom(P) ] \\
  \llparenthesis t \rrparenthesis^{\varphi,P}_n && [\alpha\mid \alpha \Rightarrow \varphi \wedge t] \\
  \llparenthesis p + q \rrparenthesis^{\varphi,P}_n && \llparenthesis p \rrparenthesis^{\varphi,P}_n + \llparenthesis q \rrparenthesis^{\varphi,P}_n \\
  \llparenthesis p \& q \rrparenthesis^{\varphi,P}_n &&
  [g~|~g \in \llparenthesis p \rrparenthesis_n^{\varphi,P}, g \in \llparenthesis q \rrparenthesis_n^{\varphi,P}] \\
  \llparenthesis p;q \rrparenthesis^{\varphi,P}_n &&
  \left(\llparenthesis p \rrparenthesis^{\varphi,P}_n \cdot \llparenthesis q\rrparenthesis^{\varphi,P}_n\right)|_n\\
  \llparenthesis p^* \rrparenthesis^{\varphi,P}_n && [] + \left(\llparenthesis p\rrparenthesis^{\varphi,P}_n \cdot  \llparenthesis p^* \rrparenthesis^{\varphi,P}_{n-i}\right)|_n \\
  \multicolumn 2 r {} & \text{where}~i = \max\{1,\min\{|g|\mid g \in \llbracket p \rrbracket^{\varphi,P}\}\}\\
  \llparenthesis \sim p \rrparenthesis^{\varphi,P}_n && [g \mid g \in \llbracket \_^* \rrbracket^{\varphi,P}_n, g \not\in \llbracket p \rrbracket^{\varphi,P}_n] \\
  \llparenthesis \langle a \rangle p \rrparenthesis^{\varphi,P}_n &&
  [g' \mid g \in g'~\doublehat{R_a}~g, g \in \llparenthesis p \rrparenthesis^{\varphi,P}]
  \end{array}
  \]
\end{figure}
...
sentences are mapped to terms in the logical language (--) via an interpreted
grammar, and these terms are in turn interpreted as propositions (sets of possible
worlds). For the latter, there are options of translation into the finite state
calculus in order to represent propositions as finite state machines (Section 3), and
representation in Haskell via lazy lists of guarded strings (Section 4).  The grammar
is a semantically interpreted multi-modal categorial grammar, consisting of a lexicon
of words, their categorial types, and interpretations in a logical lambda language.
Figure \ref{phenomenafigure} lists phenomena that are covered.
...
Figure \ref{lexiconfigure} gives illustrative lexical entries. The grammar and semantics are in
certain way optimized for a simple fragment of English concerned with clausal
...
relation, modal force, and proposition contributed by the complement.  These
complementizers introduce the dual via two negations, in order to arrive universal
modal force.  These moves are offered here as a way of constructing a compact
interpreted grammar. They can easily be reformulated in a more comprehensive
interpreted grammar of English.
...
Multimodal categories such as $\backslash_D$ and $\backslash_M$ are used to control
the derivation.  For instance the category of {\em heads} $d \backslash_D t$.  The
dummy expletive subject {\em it} has category $d$, but the phrase {\em it heads} of
category $t$ can not be formed, because $\backslash_D$ is not syntactically active as
a function.  Instead {\em it is heads} can be formed with a predicator {\em is} of
category $(d \backslash t)\slash(d \backslash_D t)$.  (This uses Lambek/Bar-Hillel
notation for slashes, so that $(d \backslash t)\slash(d \backslash_D t)$ combines
with $d \backslash_D t$ on the right to give a value that combines with $d$ on the
left to give $t$.)  Similarly {\em knows} has a category with the top-level slash
$\slash_M$, and combines to form a sentence as an argument of {\em that} or {\em
  whether}, which has a category that looks for the category of {\em know} on the
left, after combining with a complement sentence on the right.

The semantic translations in the third column of Figure \ref{lexiconfigure} use the Epik term
language, incremented with lambda.  The body of $\lambda x.0^c{\cdot}h$, which is the
semantic lexical entry for {\em heads}, is a term denoting the set of all worlds
where the coin is heads, represented as the set of all guarded strings that end with
a Boolean valuation where the primitive proposition $h$ (it's heads) is true.  There
is $\lambda x$ at the front because of a correspondence the grammar formalism uses a
correspondence between syntactic and semantic types. However, it does not bind
anything, because sentences such as {\em it isn't heads} have an expletive subject.
The body ${\Diamond}R p$ of $\lambda p.\lambda R. {\Diamond}R p$, which is the
semantic lexical entry of {\em knows}, is an Epik term denoting the pre-image of the
set of worlds $p$ according to the relation $\doublehat{R}$ between guarded strings
that is determined by the event-level relation $R$.  This is not the right semantics
for {\em Amy knows that it's heads}, because it is an existential modality ${\Diamond_R}p$,
rather than an universal modality ${\Box_R}p$.  This is corrected by the complementizer
{\em that} or {\em whether}, which introduces the dual.

Sentences are parsed with a chart parser for categorial grammar.  The semantics for
complex phrases are obtained by syntactic application of semantic translations,
accompanied by beta reduction.  Semantic terms in the parsing formalism are
expressions of untyped lambda calculus.  The grammar is set up so that lambda is
eliminated by beta reduction in the semantic term corresponding to a sentence. In
consequence, the semantic term translating a sentence is a term of the Epik language
(--).  Such a term designates a set of possible words (guarded strings) in the
possible worlds model deterimened by an Epik specification such as the one in Figure
1.  \exi{three}{a} is an English sentence with predicate conjunction and three levels
of clausal embedding.  Using the grammar and parser, the sentence is mapped to the
Epik term \exi{b}.  Using the result from Section 3, this term can be mapped in an
...
\itema  Amy knows that Bob knows that Amy knows whether it is heads and
knows that Bob does not know that Amy knows that it is tails.
\itemb
\end{examples}  
