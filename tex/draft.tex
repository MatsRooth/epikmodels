...
$\rho$ and the relation on valuations for the event $u$ are \sem{rho}
and $\sem{\zeta}^{\sem{rho}}$ as given at the bottom in \ex{6}.  Given
a state formula $\rho$, the the relation characterizing pre- and
post-conditions of an event with effect formula $\zeta$ is defined to
be $\sem{\zeta}^{\sem{\rho}}$.
...
\begin{array}{cll}
  \sem{\rho} & = & \mbox{Set of Boolean valuations of} \\
            && \mbox{the atomic tests that verify $\rho$.} \\
...
epistemic semantics.  Guarded strings over a finite alphabet {\sf{P}} are
like ordinary strings, but with truth assignments to a set $T$ of
...
parsed formulae in each). For a formula $\phi$, the set of feasible atoms is
$\mathcal A_\phi$. In the coin example, the constraint $\varphi$ is
$h\bar t + \bar h t$. Since $\bar h t \models \varphi$ and $h \bar t
\models \varphi$, they are permitted feasible atoms, and since $\bar h
\bar t$ and $h t$ do not satisfy $\phi$ they are not permitted.
...
\hspace{-2mm}\(\hat 0 ~u~ \hat 1 ~u~ \hat 2 ~d~ \hat 1 \cdot \hat 1 ~u~ \hat 2 = \hat 0 ~u~ \hat 1 ~u~ \hat 2 ~d~ \hat 1 ~u~ \hat 2 \)

\hspace{-2mm}\( \hat 0 ~u~ \hat 1 ~u~ \hat 2 ~d~ \hat 1 \cdot \hat 2 ~u~ \hat 3 = \mbox{\em{undefined}} \)

\hspace{-2mm}\(H \cdot H ~a_0~ H = H ~a_0~ H \)

\hspace{-2mm}\(T \cdot T ~a_1~ T = \mbox{\em{undefined}} \)
\end{example}
...
\beta$ to the relation $P(e)$ for every disjunct $\phi:\phi'$ such
that $\alpha \models \phi$ and $\beta \models \phi'$.  These have the
...
These operators combine to form the algebraic structure Kleene Algebra
with Tests (\textsc{Kat}), which has the algebraic signature
$\tuple{K,+,\cdot,*,\bar{},0,1}$.  We interpret each term in as a set
of guarded strings as indicated in Figure~\ref{fig:interp}. A bare
event $e$ is interpreted as the set of guarded strings allowed by
$\varphi$ and $P$. The operation $p + q$ is set union. The constant
$0$ is the empty set.  The constant 1 is the set of all truth
assignments obeying the constraint on truth assignments,
i.e. $\{10,01\}$ in the coin example.  The operation $\cdot$ is fusion
product raised to sets: $x \cdot y$ is the set of all defined fusion
products of an element of $x$ with and element of $y$.  The operation
$*$ is Kleene star, with $x* = \cup_{i\geq 0}x^i$, where $x^i$ is the
$i$-times product of $x$ with itself and $x^0=1$.  Subsets of $1$
indicated by $t$ are also elements of $K$, and these form the Boolean
algebra of tests, denoting all atoms admitted by the constraint
$\varphi$ and the boolean test $t$.  We ``overload'' $\cdot$ to
further indicate boolean conjunction, $+$ to further indicate boolean
disjunction, and $0,1$ to indicate the zero and one elements of the
boolean algebra. The overbar operation denotes complement in this
Boolean algebra: $\overline t$ denotes all atoms $\alpha$ such that
$\alpha \Rightarrow \neg t$.  Other boolean operators (such as
implication $\Rightarrow$) are encoded as syntactic sugar in the
standard way. 


To this we add an unary operation $\Diamond_a$ for each agent $a$, and a
unary complent operation $\cdot^{c}$ on terms.  This results in the
signature $\tuple{K,+,\cdot,*,\bar{},0,1,{}^c,\Diamond_a,\Diamond_b}$
for epistemic KAT with two agents.  We interpret the epistemic
epistemic modality $\Diamond_a$ using Kripke semantics, as pre-image
relative to a fixed relation $R_a$ between guarded strings,
$\Diamond_a~x = \{u \mid \exists v. v \in x \wedge u \mathrel{R_a} v\}$.
Here $u$ and $v$ are guarded strings, while $x$ is an element of $K$.
...
$\hat{R_a}$ between decorated events for a given $\varphi$ and $P$,
see \ex{Rhat}, and again to generalized to a relation $\doublehat{R}$
on arbitrary guarded strings by relational Kleene star (as opposed to
the Kleene star from aboce) , as defined in \ex{Rhathat}. The
operation $\cdot$ in the definition is KAT product, which enforces
matching of tests.
...
\hspace{6mm} x \in \llbracket u_1 \cdot ... \cdot u_n \rrbracket^{\varphi,P}\hspace{1mm} \wedge \\
\hspace{6mm} y \in \llbracket v_1 \cdot ... \cdot v_n \rrbracket^{\varphi,P} \hspace{2mm} \}
\end{array}
\)
\end{example}
...
\begin{figure}
  ...
  \llbracket 1 \rrbracket^{\varphi,P} && \{\alpha \mid \alpha \Rightarrow \varphi\}\\
  \llbracket e \rrbracket^{\varphi,P}&& \{\alpha \cdot e \cdot \beta \mid
  ...
  \llbracket t \rrbracket^{\varphi,P} &&  \{\alpha \mid \alpha \Rightarrow \varphi \wedge t\}\\
  \llbracket{p+q}\rrbracket^{\varphi,P} && \llbracket{p}\rrbracket^{\varphi,P} \cup \llbracket q \rrbracket^{\varphi,P}\\
  \llbracket{p \& q}\rrbracket^{\varphi,P} && \llbracket{p}\rrbracket^{\varphi,P} \cap \llbracket q \rrbracket^{\varphi,P}\\
  \llbracket{p;q}\rrbracket^{\varphi,P} && \llbracket p \rrbracket^{\varphi,P} \cdot \llbracket{q}\rrbracket^{\varphi,P}\\
  \llbracket p^*\rrbracket^{\varphi,P} && \displaystyle\bigcup_n {\underbrace{\llbracket p \rrbracket^{\varphi,P}\cdot \ldots \cdot \llbracket p \rrbracket^{\varphi,P}}_{n}} \\
  \llbracket \sim p \rrbracket^{\varphi,P} && \llbracket \_^*\rrbracket^{\varphi,P} \setminus \llbracket p \rrbracket^{\varphi,P} \\
  \llbracket \langle a \rangle~p \rrbracket && \doublehat{R_a}\llbracket p \rrbracket
...
Summing up, given an Epik program with $n$ agents, we construct a
concrete guarded string model
$\tuple{K,+,\cdot,*,\bar{},0,1,{}^c,\Diamond_{a_1},...,\Diamond_{a_n}}$.
The elements of $K$ are interpreted as propositions.  $0^c$ is the set
of worlds, and it may be countably infinite.  $\Diamond_{a_i}$ is an
epistemic modality for the $i$th agent.  Or referring to the Kripke
relations $\doublehat{R}_i$, the construction defines a multi-agent
Kripke frame $\tuple{0^c,\doublehat{R}_1,...,\doublehat{R}_n}$
(usually a countable one) from an Epik specification. The frame
consists of a set of worlds, and an epistemic-alternative relation for
each agent.  These models are used as a target for natural-language
interpretation in Section 5 and Section 6, where we obtain semantic
values such as \sem{Amy knows that it's heads and Bob knows that Amy
  knows whether it is heads or tails, and does not know that it's
  heads} as elements of $K$. Concretely the proposositions are sets of
guarded strings (usually countable ones), construed as sets of worlds
as they figure in possible worlds semantics for natural language.
...
\begin{figure}
\begin{tabular}{lll}
agents & $a,b...$ & $a {:}{:}{=} \, \tuple{\mbox{atomic agents}}$
\end{tabular}

\begin{tabular}{lll}
tests & $T,S...$ & $T{:}{:}{=} \, \begin{array}[t]{l}\tuple{\mbox{atomic tests}}\,|\,0\,|\,1\,|\\ T{+}S\;|\;T\,{;}\,S\;|\;!T\end{array}$
\end{tabular}

\begin{tabular}{lll}
propositions & $p,q...$ & \(p{:}{:}{=} \,
\begin{array}[t]{l}\tuple{\mbox{atomic events}}\,|\,\_\, |\\
T \,|\, 
p{+}q \,|\,
p\mbox{\&}q \,|\, \\
p{;}q \;|\;
...
\caption{The language of Epik terms. Propositional terms are interpreted
  as sets of guarded strings.}
