...
the vectors $\bar q \bar p$, $\bar q p$, $q \bar p$, $q p$.  In the
coin example, we get $h\bar t$ and $\bar h t$, as well as the bogus
atoms $\bar h \bar t$ and $ht$!  To preclude bogosity, the
\texttt{constraint} declaration in Figure~\ref{epikfigure} describes
the set of \emph{feasible atoms}, which we parse into a formula
$\varphi$ (if there are multiple \texttt{constraint declarations},
$\varphi$ is the conjunction of the parsed formulae). For a formula
$\phi$, the set of feasible atoms is $\mathcal A_\phi$. In the coin
example, the constraint $\varphi$ is $h\bar t + \bar h t$. Since $\bar
h t \models \varphi$ and $h \bar t \models \varphi$, they are
permitted feasible atoms, and since $\bar h \bar t$ and $h t$ do not
satisfy $\phi$ they are not permitted.
...
\hspace{-2mm}\( \bar p q ~u~ \bar p q ~u~ p \bar q ~d~ \bar p q \cdot \bar p q ~u~ p \bar q = \bar p q ~u~ \bar p q ~u~ p \bar q ~d~ \bar p q ~u~ p \bar q \)

\hspace{-2mm}\( \bar p \bar q ~u~ \bar p q ~u~ p \bar q ~d~ \bar p q \cdot p \bar q ~u~ pq = \mbox{\em{undefined}} \)

\hspace{-2mm}\(\bar h t \cdot \bar h t ~a_0~ \bar h t = \bar h t ~a_0~ \bar h t \)

\hspace{-2mm}\(\bar h t \cdot h \bar t ~a_1~ h \bar t = \mbox{\em{undefined}} \)
\end{example}
...
event is flanked by compatible atoms. In standard KATs, this is  as defined in an Epik
program by the event declaration and the constraint declaration.
These have the status of event types, in that they can ``happen'' in
different possible worlds. Here happening corresponds to incrementing
the world with some compatible element of the event type.  Where $e$
is a bare event symbol, $\llbracket e \rrbracket^{\varphi,P}$ is the
...
\begin{array}[t]{l|c}
  ...
  u & \{ \bar p \bar q ~u~ \bar p q, \bar p q ~u ~p \bar q,  p \bar q ~u~ pq\}\\
  d & \{ \bar p q ~d~ \bar p \bar q, p \bar q~d~\bar p q, p q~d~ p \bar q\}\\
  a_1 & \{h \bar t ~a_1~ h \bar t \}\\
  a_0 & \{t \bar h ~a_0~ t \bar h\}\\
  b_1 & \{h\bar t ~b_1~ h \bar t \}\\
  b_0 & \{t \bar h ~b_0~ t \bar h \}\\
\end{array}
\)
\end{example}
