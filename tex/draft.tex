...
of $a_1$, and $\overline{pq}$ being the precondition of
$u$. Juxtaposition is used for Boolean conjunction, and the overbar
for Boolean negation. Events come as well with a relation between
...
change the state.  The third line says that $u$ increments the floor,
and can happen only on floors 0, 1, and 2.  The fourth line describes
$d$ in similar terms.  Plus is disjunction.
...
a simply Boolean formula, in this case \exi{effect}{a}.  Where
{\sf{B}} is a set of atomic tests and $\rho$ is a state constraint
over {\sf{B}}, ${\cal{A}}_{\mbox{\sf\tiny{B}}}^{\rho}$ is the set of
valuations of {\sf{B}} that make formula $\rho$ true. Following
\cite{kozen2001automata}, we call the valuations
\emph{atoms}.
...
$\varphi$ and the relation on valuations for the event $u$ are
$\mathcal A_{\mathsf B}^\varphi$ and
$\llbracket\zeta\rrbracket^\varphi$ as given at the bottom in \ex{6}.
Given a state formula $\rho$, the the relation characterizing pre- and
post-conditions of an event with effect formula $\zeta$ is defined to
be $\sem{\zeta}^{\sem{\rho}}$.

\begin{example}   \label{effect}
\begin{tabular}[t]{l}
State formula: $\varphi = h\bar{t} + \bar{h}t$ \\
Effect formula for $a_1$: $\zeta = h \mathbin{:} h$ \\
...
\begin{figure}
  ...
  \varphi,\psi &::=& a \mid0 \mid 1 \mid\varphi{+}\psi \mid \varphi~\psi \mid \bar{\varphi} \\
  \multicolumn {3} l {\textrm{effect formulas}}\\
  \zeta,\eta & ::= & \varphi \mathbin{:} \sigma \mid \zeta{+}\eta
    \mid \zeta \mathbin{\&} \eta \mid \bar{\zeta}
  \end{array}\]
  
\[
\begin{array}{cll}
  \llbracket\psi_1\mathbin{:}\psi_2\rrbracket^\varphi & = &
  \mathcal A^{(\varphi~\psi_1)}_{\mathsf{B}}\times\mathcal A^{(\varphi~\psi_2)}_{\mathsf{B}}\\[.1em]
...
from $E$.  In the algebra described by Figure \ref{epikfigure}, $E$ is
the set of events $\{a_1,a_0,b_1,b_0\}$, and in the elevator example,
$\{u,d\}$.  In the elevator example, $T$ is $\{p,q\}$, and in the coin
example it is $\{h,t\}$.  We write atoms as the boolean formulae that
uniquely specify the truth assignment. In the elevator example, we
have atoms $\bar q \bar p$, $\bar q p$, $q \bar p$, $q p$, for
notational simplicity we will write these as $\hat0$, $\hat1$,
$\hat2$, $\hat3$ respectivelyto indicate the floor each atom
represents. In the coin example, we get $h\bar t$ and $\bar h t$ (for
which we use the shorthand $H$ and $T$), as well as the bogus atoms
$\bar h \bar t$ and $ht$!  To preclude bogosity, the
\texttt{constraint} declaration in Figure~\ref{epikfigure} describes a
state formula $\varphi$ which denotes the set of \emph{feasible atoms}
$\mathcal A_{\mathsf B}^\varphi$.
...
a function $\mathcal E$ from agents to relations on atoms that specify
the effects each action can have. Notably for every declaration
$\texttt{event}~e~\zeta$, we have $\mathcal E^\varphi(e) =
\llbracket\zeta\rrbracket^\varphi$. We can lift $\mathcal E^\varphi$
to $\hat{\mathcal E}$ which decorates the events with their permitted
atoms, that is $\hat{\mathcal E}^\varphi(e) = \{\alpha~e~\beta \mid
\alpha, \beta \in \mathcal E^\varphi (e) \}$. See the examples in
\ex{events1}.
...
\(
\begin{array}[t]{l|c}
  e &  \hat{\mathcal E}^\varphi(e) \\ \hline
...
\hspace{6mm} x \in \llbracket u_1 ; ... ; u_n \rrbracket^{\varphi,{\mathcal{E}}}\hspace{1mm} \wedge \\
\hspace{6mm} y \in \llbracket v_1 ; ... ; v_n \rrbracket^{\varphi,{\mathcal{E}}} \hspace{2mm} \}
\end{array}
\)
\end{example}
...
Figure~\ref{fig:epik-grammar} depicts the syntax of Epik's formal term
language. As in KAT, $0$ is the null element, $1$ is the unit element,
$p + q$ indicates disjuction, $p;q$ (or simply $pq$) indicates
sequence, the boolean operators for tests $\varphi$ are the standard
ones (as in Figure~\ref{fig:effect-syntax}), and ${p}^*$ indicates
iteration of $p$. We add some additional operations to reason about epistemic
...
\begin{figure}
  ...
  \multicolumn 2 l {\textrm{terms}} & e \in \mathsf{P} \\
  p,q & ::=& e~|~0~|~1~|~\phi~|~p+p~|~p;p~|~p^*~|\neg p~|\Diamond_a~p
  \end{array}\]

  Derived Operators\\[.1em]
  \(\Box_a p \triangleq \neg\Diamond_a\neg p\) \hfill \(p\&q \triangleq \neg(\neg p + \neg q)\) \hfill \(\_ \triangleq \sum\mathsf P\)
...
\rrbracket^{\varphi,{\mathcal{E}}}$ interprets a term $p$ as a set of
guarded strings consistent with state constraints $\varphi$ and effect
relations ${\mathcal{E}^\varphi}$. A bare event $e$ is interpreted as
the set $\hat{\mathcal E}^\varphi(e)$. The constant $0$ is interpreted
as the empty set.  The constant $1$ is interpreted as the set
$\mathcal A^\varphi_{\mathsf B}$, i.e. $\{h\bar t, \bar h t\}$ in the
coin example. The operation $p + q$ is interpreted as the union of the
...
the denotation of $p$ with itself. Subsets of $1$ indicated by
propositions $\psi$ are also elements of $K$, and these form the
Boolean algebra of tests, denoting all atoms admitted by the
constraint $\varphi$ and the boolean test $\psi$.
...
$\Diamond_a~p = \{u \mid \exists v. v \in \llbracket
p\rrbracket^{\phi, \mathcal E} \wedge u \mathrel{\hat{R_a}} v\}$.  Here $u$
and $v$ are guarded strings, while $x$ is an element of $K$.
...
\begin{figure}
  ...
  \llbracket 0 \rrbracket^{\varphi,{\mathcal{E}}} && \{\} \\  
  \llbracket 1 \rrbracket^{\varphi,{\mathcal{E}}} && \mathcal A^\varphi_{\mathsf B}\\
  \llbracket e \rrbracket^{\varphi,{\mathcal{E}}}&& \hat{\mathcal E}^\varphi(e) \\
  \llbracket \psi \rrbracket^{\varphi,{\mathcal{E}}} &&  \mathcal A^{\varphi~\psi}\\
  \llbracket{p+q}\rrbracket^{\varphi,{\mathcal{E}}} && \llbracket{p}\rrbracket^{\varphi,{\mathcal{E}}} \cup \llbracket q \rrbracket^{\varphi,{\mathcal{E}}}\\
  \llbracket{p;q}\rrbracket^{\varphi,{\mathcal{E}}} && \llbracket p \rrbracket^{\varphi,{\mathcal{E}}} \diamond \llbracket{q}\rrbracket^{\varphi,{\mathcal{E}}}\\
  \llbracket p^*\rrbracket^{\varphi,{\mathcal{E}}} && \displaystyle\bigcup_n {\underbrace{\llbracket p \rrbracket^{\varphi,{\mathcal{E}}}\diamond \ldots \diamond \llbracket p \rrbracket^{\varphi,{\mathcal{E}}}}_{n}} \\
  \llbracket \neg p \rrbracket^{\varphi,{\mathcal{E}}} && \llbracket \_^*\rrbracket^{\varphi,{\mathcal{E}}} \setminus \llbracket p \rrbracket^{\varphi,{\mathcal{E}}} \\
  \llbracket \Diamond_a~p \rrbracket && \{x \mid x \mathbin{\hat{R_a}} y, y \in \llbracket p \rrbracket^{\varphi,\mathcal E}\}
...
denoted by $p$. Note that $\llbracket 0^c
\rrbracket^{\varphi,{\mathcal{E}}} =
\llbracket\_^*\rrbracket^{\varphi,{\mathcal{E}}}$.

Summing up, given an Epik program with $n$ agents, we construct state
constraints $\varphi$, effect relations ${\mathcal{E}}$, and a concrete guarded
...
$K$. Concretely the propositions are sets of guarded strings (usually
countable ones), construed as sets of worlds as they figure in
possible worlds semantics for natural language.
...
with itself (the $0$-times concatenation product is $\llbracket 1
\rrbracket^{\varphi,\mathcal E}$). This is an operation in the finite
...
the interpretation function, $\llparenthesis p \rrparenthesis^{\varphi,{\mathcal{E}}}_n$ on an
...
\begin{figure}
  ...
  \llparenthesis 1 \rrparenthesis^{\varphi,{\mathcal{E}}}_n && [\alpha \mid \alpha \leq \varphi] \\
  \llparenthesis e \rrparenthesis^{\varphi,{\mathcal{E}}}_n && [\alpha \cdot e \cdot \beta \mid (\alpha, \beta) \in {\mathcal{E}}(e), \alpha\vee \beta \Rightarrow \varphi] \\
  \llparenthesis \_ \rrparenthesis^{\varphi,{\mathcal{E}}}_n && \sum [\llparenthesis e \rrparenthesis^{\varphi,{\mathcal{E}}} \mid e \in \dom({\mathcal{E}}) ] \\
  \llparenthesis t \rrparenthesis^{\varphi,{\mathcal{E}}}_n && [\alpha\mid \alpha \Rightarrow \varphi \wedge t] \\
  \llparenthesis p + q \rrparenthesis^{\varphi,{\mathcal{E}}}_n && \llparenthesis p \rrparenthesis^{\varphi,{\mathcal{E}}}_n + \llparenthesis q \rrparenthesis^{\varphi,{\mathcal{E}}}_n \\
  \llparenthesis p \& q \rrparenthesis^{\varphi,{\mathcal{E}}}_n &&
  [g~|~g \in \llparenthesis p \rrparenthesis_n^{\varphi,{\mathcal{E}}}, g \in \llparenthesis q \rrparenthesis_n^{\varphi,{\mathcal{E}}}] \\
  \llparenthesis p;q \rrparenthesis^{\varphi,{\mathcal{E}}}_n &&
  \left(\llparenthesis p \rrparenthesis^{\varphi,{\mathcal{E}}}_n \cdot \llparenthesis q\rrparenthesis^{\varphi,{\mathcal{E}}}_n\right)|_n\\
  \llparenthesis p^* \rrparenthesis^{\varphi,{\mathcal{E}}}_n && [] + \left(\llparenthesis p\rrparenthesis^{\varphi,{\mathcal{E}}}_n \cdot  \llparenthesis p^* \rrparenthesis^{\varphi,{\mathcal{E}}}_{n-i}\right)|_n \\
  \multicolumn 2 r {} & \text{where}~i = \max\{1,\min\{|g|\mid g \in \llbracket p \rrbracket^{\varphi,{\mathcal{E}}}\}\}\\
  \llparenthesis \sim p \rrparenthesis^{\varphi,{\mathcal{E}}}_n && [g \mid g \in \llbracket \_^* \rrbracket^{\varphi,{\mathcal{E}}}_n, g \not\in \llbracket p \rrbracket^{\varphi,{\mathcal{E}}}_n] \\
  \llparenthesis \langle a \rangle p \rrparenthesis^{\varphi,{\mathcal{E}}}_n &&
  [g' \mid g \in g'~\doublehat{R_a}~g, g \in \llparenthesis p \rrparenthesis^{\varphi,{\mathcal{E}}}]
  \end{array}
  \]
\end{figure}
