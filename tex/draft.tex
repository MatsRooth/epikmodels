...
$\tuple{K,+,\cdot,*,\bar{},0,1}$.  To this we add an unary operation
$\Diamond_a$ for each agent, and a unary complent operation
$\cdot^{c}$ on propositions.  This results in the signature
$\tuple{K,+,\cdot,*,\bar{},0,1,{}^c,\Diamond_a,\Diamond_b}$ for
epistemic KAT with two agents.

We interpret each term in as a set of guarded strings as indicated in
Figure~\ref{fig:interp}. A bare event $e$ is interpreted as the set of
guarded strings allowed by $\varphi$ and $P$. The operation $p + q$ is
set union. The constant $0$ is the empty set.  The constant 1 is the
set of all truth assignments obeying the constraint on truth
assignments, i.e. $\{10,01\}$ in the coin example.  The operation
$\cdot$ is fusion product raised to sets: $x \cdot y$ is the set of
all defined fusion products of an element of $x$ with and element of
$y$.  The operation $*$ is Kleene star, with $x* = \cup_{i\geq 0}x^i$,
where $x^i$ is the $i$-times product of $x$ with itself and $x^0=1$.
Subsets of $1$ indicated by $t$ are also elements of $K$, and these
form the Boolean algebra of tests, denoting all atoms admitted by the
constraint $\varphi$ and the boolean test $t$.  We ``overload'' $\cdot$
to further indicate boolean conjunction, $+$ to further indicate
boolean disjunction, and $0,1$ to indicate the zero and one elements
of the boolean algebra. The overbar operation denotes complement in
this Boolean algebra: $\overline t$ denotes all atoms $\alpha$ such
that $\alpha \Rightarrow \neg t$.  Other boolean operators (such as
implication $\Rightarrow$) are encoded as syntactic sugar in the
standard way. The complement operation ${p}^c$ is complement at the
level of sets of guarded strings, defined to every set the universe
$\_^*$ except those denoted by $p$. Finally, the epistemic modality
$\Diamond_a$ is interpreted using Kripke semantics, as pre-image
relative to a fixed relation $R_a$ between guarded strings,
$\Diamond_a x = \setabs{u}{\exists v.\element{v}{x} \wedge u R_a v}$.
...
agent specification as in Figure \ref{epikfigure}. An an agent
specification pairs each bare event with a set of bare events, and so
determines a relation between bare events, call it relation $R_a$ for
an agent $a$.  This determines a relation $\hat{R_a}$ between
decorated events, see \ex{Rhat}.  It is generalized to a relation
$\doublehat{R}$ on arbitrary guarded strings by relational Kleene
star, as defined in \ex{Rhathat}. The operation $\cdot$ in the
definition is KAT product, which enforces matching of tests.  The
number of terms $n$ is construed as satisfying $n\geq 1$.  {\em What
  is desired for $n=0$?}
