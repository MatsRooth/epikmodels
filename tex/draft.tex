...
epistemic KAT is $\tuple{K,+,\cdot,*,\bar{},0,1,\neg,\Diamond_1
  ... \Diamond_n}$. Figure \ref{termfigure} defines the syntax of the language, but
using juxtaposition in place of the product symbol $\cdot$.
As illustrated in \ex{interpretation1}, terms in this language are used to
represent the proposositional semantic values of English sentences.\footnote{See Sections
6 and 7 for explanation and further examples.  }
...
\begin{figure}
  ...
  \multicolumn{3}{l}{\mbox{state terms $\sigma$ as in Figure 1}} \\
  \multicolumn 2 l {\textrm{events}} & e \in \mathsf{P} \\
...
Each term in the language is interpreted as a set of guarded strings.  Where $p$ is
a term, $\sem{p}^{\test,\event,\varphi,\zeta}$ is the interpretation of the term.
The superscript captures dependence on an Epik specification.





interprets a term $p$ as a set of
...
\begin{figure}
  ...
  \sem{ a }^{\epikspec} && \mathcal A^{a\varphi}_{\mathsf B}\\
