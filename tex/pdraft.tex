...
\citep{hughes1996new}. The axioms in \ex{valid-axioms}
...
\begin{example}\label{conditionally-valid-axioms}
  ...
  \textbf{4} & \Box_a p \to \Box_a\Box_a p & \text{if } \hat{R}_a \text{ is idempotent}
  \end{array}\)
\end{example}
...
modal axioms, \textbf{B} and \textbf{5}, are similar.
...
\texttt{agent} relation from Figure~\ref{epikfigure} is implemented in Fst.
...
\section{Discussion}
Epistemic \textsc{Kat} is designed for use in linguistic semantics research, for
compultational linguistics research on model-theoretically grounded semantics,
and for students in graduate course modules on intensional formal semantics.
...
it would be possible to use Epik with other frameworks that support lambda
