...
\begin{abstract}
  ...
  as sets of guarded string models in an epistemic extension of Kleene Algebra
  with Tests (\textsc{Kat}). The account is framed as a formal language Epik
  (Epistemic \textsc{Kat}) for defining such models. The language is implemented
  by translation into the finite state calculus, and alternatively by modeling
  propositions as lazy lists in Haskell. The syntax-semantics interface for a
...
Enter Epik, which is a systematic way of defining such models, and can be
...
Epik leverages these algebrae to define a mathematical model
...
{\em Related Work.} Epik synthesizes five antecedents.
...
table under a cup, heads up. The coin could be heads-up $H$ or tails-up $T$, and
...
after the events of $w_2$, event $b_1$ occurs, which corresponds to, in the
initial world, Amy surreptitiously peeking at a heads-up coin, and then Bob
doing the same.
...
For example, $\apH\atTH$ would require the coin to simultaneously heads-up and
tails-up. To mechanize this, events in Epik come with Boolean pre-conditions and
post-conditions. In the running example, the Boolean variables $h$ (the coin is
heads-up) and $t$ (the coin is tails-up) represent the coin's state. Then, since
Amy can only observe heads when the coin is heads-up, event $\apH$ has the
invariant condition $h$. Similarly, Amy can only turn the coin from tails to
heads if it shows tails, so $\atTH$ has the tails-up precondition $t$ and the
heads-up postcondition $h$.
...
h$. Effect formulae are interpreted relations on boolean valuations, as defined
...
\emph{state formula}, shown in \ref{effect}, and demonstrated for $\apH$ in
\ref{effect}. The state formula, also shown in \ref{effect}, says that the coin
is either heads-up or tails-up, but not both\footnote{Two Booleans constrained
  by a state formula, is a more illustrative example than using a single boolean
  and modelling tails as $\bar h$.}. Here juxtaposition represents conjuction,
$+$ is disjunction, and overbar is negation.

With the state formula in hand, $\apH\atTH$ is evidently contradictory, the
post-condition $h$ of $\apH$ is incompatible with the pre-condition $t$ of
...
\mbox{effect formula $\zeta_{a_1}$ for $a_1$} &  h \mathbin{:} h \\
 \mathcal{A}^\varphi_{\mathsf B} & \{h\bar{t},\bar{h}t\}   \\
\sem{\zeta_{a_1}}^\varphi & \{\pair{h\bar{t}}{h\bar{t}}\}   \\
\end{array}\)
\end{example}

Thus far, the presentation is closely related to Kleene Algebra with Hypotheses
...
Rather than rely on general techniques ~\cite{doumane2019kleene,
  hardin2002elimination, kozen1997kleenecompleteness}, we relalize our specific
hypotheses directly.

The key facet that differentiates Epik from \textsc{Ka+H} is the epistemic
modality, which cannot be naively represented using universal
hypotheses\footnote{The existence of this encoding is an interesting future
  topic}. Sections~\ref{sec:model} and~\ref{sec:logic} discuss the
model-theoretic and logical development of the epistemic operators.
