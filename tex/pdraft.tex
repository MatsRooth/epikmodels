...
\begin{abstract}
  ...
  with Tests (\textsc{Kat}). The account is framed as a formal language
  Epi\textsc{Kat} (Epistemic \textsc{Kat}) for defining such models. The
  language is implemented by translation into the finite state calculus, and
...
Epi\textsc{Kat} leverages these algebrae to define a mathematical model
...
{\em Related Work.} Epi\textsc{Kat} synthesizes five antecedents.
...
tails-up. To mechanize this, events in Epi\textsc{Kat} come with Boolean pre-conditions and
...
Epi\textsc{Kat} is first and foremost a multi-agent epistemic logic, with epistemic
operators that cannot naively be represented as
hypothesis\footnote{Characterizing this relationship precisely is a future
  research question.}. Aside from formalizing the metaphysical modality seen so
far, Sections~\ref{sec:model} and~\ref{sec:logic} develop the epistemic
modality.
...
The discussion of \ex{coin2} mentioned building worlds by incrementing worlds
with events and maintaining the pre-and-post conditions. This is accomplished in
guarded string models with the fusion product ($x \diamond y$), a partial
operation that combines two guarded strings $x$ and $y$, subject to the
condition that the atom at the end of the $x$ is identical to the atom at the
start of $y$. \ex{fusion1} gives some examples.
...
state and effect formulae. \ex{wf1} defines the \emph{well-formed} guarded
...
which, like $\bpT\cpH$, are bogus. These contradictions are filtered by post-
and pre-conditions of events in the alternative world, so that the set of
...
is a concrete guarded string interpretation for the signature of epistemic
\textsc{Kat}, with operations as in Figure \ref{fig:interp2}. This provides a
...
\section{Discussion}
Epi\textsc{Kat} is designed for use in linguistic semantics research, for
...
Currently, the grammar covers statives, {\em that-} and {\em
  whether}-complements, negation, and predicate and sentence conjunction.
...
are hidden in the primitive event symbols such as $a_1$ and $b_1$. This
...
axioms, coalgebrae, and relations to other logics, warrant further investigation.
