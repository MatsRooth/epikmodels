...
\begin{abstract}
  ...
  account is framed as a formal language Epik (Epistemic \textsc{Kat}) for
...
The approach taken here synthesizes five antecedents.
...
to express intensions \cite{fernando2017intensions}.  Work on {\em
  finite state intensional semantics} has investigated how to do the
...
A running example of an event-sequence model is {\em The Concealed Coin}. Amy
and Bob are seated at a table. There is a coin on the table under a cup, heads
up. The coin could be heads-up $H$ or tails-up $T$, and neither agent knows
which it is. This initial situation is possible world $w_1$. We can construct
additional worlds by sequencing events like those shown in \ex{coin1}.
...
The worlds $w_1,\ldots,w_4$ in \ex{coin2} are examples constructed from the
events in \ex{coin1}. Juxtaposition indicates the order and occurence of events,
for example $w_3$ indicates that after the events of $w_2$, event $b_1$ occurs,
fully, this indicates beginning in world $w_1$, Amy surreptitiously peeking at
a heads-up coin, and then Bob doing the same.
...
The truth values for English sentences are observed at the end of the event
sequences specified by $w_1,\ldots, w_4$, where $0$ stands for falsity and $1$
for truth.
...
\ex{effect}.  Where {\sf{B}} is a set of state primitives and $\varphi$ is a
 constraint over {\sf{B}},
${\cal{A}}_{\mbox{\sf\tiny{B}}}^{\varphi}$ is the set of valuations of
{\sf{B}} that make formula $\varphi$ true. Valuations are called atoms,
...
In the algebra described by Figure \ref{epikfigure}, ${\sf{P}}$ is the set of
events $\{\apH,\apT,\bpH,\bpT,\atTH,\atHT,\btTH,\btHT,o\}$, and ${\sf{B}}$ is
...
this construction, $w_1=H$, $w_2=Ha_1H$, $w_3=Ha_1Hb_1H$, and
...
model for \textsc{Kat} are sets of guarded strings.  In our application, these
...
Consider a scenario like the one from Figure 1, but with an additional agent
Cal. The base world $T b_0 T c_0 T$ is one where the coin is tails, and first
Bob looks at tails, and them Cal looks at tails. The first event $b_0$ has the
alternatives $b_0$ and $b_1$ for Amy, and the second event $c_0$ has the
alternatives $c_0$ and $c_1$ for Amy. This results in four combinations
$b_0c_0$, $b_0c_1$, $b_1c_0$, and $b_1c_1$. But some of these worlds are bogus,
such as $b_0c_1$, which says that Bob peeked at the coin and saw tails, and Cal
peeked at the same coin and impossibly saw heads. These contradictions are
filtered by post- and pre-conditions of events in the alternative world, so that
the set of alernatives for Amy in $T b_0 T c_0 T$ is $\{T b_0 T c_0 T,H b_1 H
c_1 H\}$, with two world-alternatives instead of four.
...
epistemic \textsc{Kat} is $\tuple{K,+,\cdot,*,\bar{},0,1,\neg,\Diamond_1,
...
is a concrete guarded string interpretation for the signature of epistemic \textsc{Kat}, with
...
for natural-language interpretation in Section~\ref{sec:categorial-grammar}.
...
here to construct of a model for epistemic \textsc{Kat} inside the finite state
...
\end{hangingpar}
\end{tcolorbox}
\caption{\raggedright Definition in Fst of \textsc{Kat} product and star.
...
difference between the set of all worlds and the argument.
...
\textsc{Kat} Kleene plus is defined in a similar way using Kleene plus in the string algebra,
with checks for equality of atoms and deletion of atoms. 
\textsc{Kat} Kleene star is defined from \textsc{Kat} Kleene plus and the multiplicative identity,
...
The epistemic alternative relation on worlds for an agent is then defined as the \textsc{Kat}
...
conjunction.\marginpar{Conjunction missing!} Figure \ref{lexiconfigure} gives
...
\begin{figure}[!t]
  ...
  \newcommand{\hdrspc}{0.3em}
    ...
    \caption{Categorial grammar lexicon, showing word form (column 1), a
      categorial type (column 2), and a semantic translation in Epik extended
...
\section{Discussion}
Epistemic \textsc{Kat} is designed for use in research in linguistic semantics,
for research in computational linguistics on model-theoretically grounded
...
additional linguistic phenomena, such as tense and perfective aspect
\exi{extension}{a}, and the combination of metaphysical modality and prospective
aspect \exi{extension}{b}.
...
Epik is closely related to Kleene Algebra with Hypotheses
\textsc{Ka+H}~\cite{cohen1994hypotheses}, which permits user-specification of
futher equations beyond the axioms of \textsc{Ka}. In practice, these hypotheses
are used to express domain-specific properties of the events, such as the
commutativity. We can model both the state formulae and the event formulae as
\textsc{KA+H} hypotheses. Specifically, the state formula $\varphi$ can be
thought of as a hypothesis $\varphi \equiv 1$, and an event formula $\zeta_a$
for event $a$, is simply the hypothesis $a \equiv \sum_{\varphi,\varphi' \in
  \llbracket \zeta_a \rrbracket} \varphi;a;\varphi'$. In general, the Horn
theory of \textsc{KA+H} is undecidable, however many special cases
are~\cite{doumane2019kleene, hardin2002elimination,
  kozen1997kleenecompleteness}.

The key facet that differentiates Epik from \textsc{Ka+H} is the modality. Each
alternative relation represents a set of uni-directional rewrites that must be
applied for that agent, rather than the universal, optional rewrites represented
by the \textsc{Ka+H} hypotheses. It may be possible to encode Epik's alternative
relations into \textsc{Ka+H} hypotheses, but such an encoding is future work.
...
characterizations of epistemic \textsc{Kat}s, e.g. sound and complete axioms
and coalgebrae.

Source code, examples, and instructions for Epik are available at
