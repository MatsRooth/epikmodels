...
In Fst, we use the Kleene closure of the concatenation product
operation on relations to lift a relation on events for an agent $a$,
$R_a$, to the corresponding relation on worlds.  First define the
concatenation product $R~S$ of two relations $R$ and $S$ to be the set
of pairs of the form $\pair{x_1x_2}{y_1y_2}$, where $x_1 \mathrel R
y_1$, and $x_2 \mathrel S y_2$. Then $R+$ is the closure of $R$ with
respect to this operation. Figure \ref{relclosure} defines the
...
Figure~\ref{fig:list-interp}. First note that when $n=0$, the
...
BBDs, which concisely represent boolean functions~\cite{BBDS}.
...
term translating a sentence is an Epik term.  Such a term designates a
set of possible words (guarded strings).  By way of example,
\exi{three}{a} is an English sentence with conjunction and several
levels of clausal embedding.  Using the grammar and parser, the
sentence is mapped to the term in \exi{three}{b}. Then \exi{three}{c}
shows a simplified logical from constructed form \exi{three}{b} using
simply logical equivalences. This term is compiled in an
...
of worlds. \ehc{Is this right anymore? the formula has changed
  drastically}. In this way the methodology ``directly'' represents
the set of worlds denoted by \exi{three}{a}.
...
are straightforward extensions to additional semantic phenomena, such
