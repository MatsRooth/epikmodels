...
example, \exi{three}{a} is an English sentence with 
conjunction and several levels of clausal embedding.  Using the
grammar and parser, the sentence is mapped to the term in
\exi{three}{b}. The negations are propositional. The double
negation, while logically redundant, comes syntactically from the grammar. This term
is compiled in an implementation of the finite state calculus to a
finite state machine with six states and eight arcs, which accepts a
countably infinite set of worlds.  In this way the methodology
``directly'' represents the set of worlds denoted by \exi{three}{a}.

\begin{examples} \label{three}
\itema Its tails and Amy knows that Bob knows that Amy knows whether its heads.
% (_*;t) & ~ <amy> ~ ~ <bob> ~((~ (<amy> ~ (_*;h))) + ~ (<amy> (_*;h)) )
\itemb \( \displaystyle
\bullet^*t \wedge \neg \Diamond_{\text{Amy}} \neg \neg \Diamond_{\text{Bob}} \neg(
\neg \Diamond_{\text{Amy}} \neg \bullet^*h 
+
\neg \Diamond_{\text{Amy}} \bullet^*h 
)
\)
\end{examples}
Sentence \exi{entail}{a} is assigned the logical form \exi{entail}{b} by the grammar.
Logical relations between propositions are checked in the finite state calculus
by checking set-theoretic relations between sets of worlds.
In this case, the propositions $p_{11}$ (from \ex{three}) and
$p_{12}$ (from \ex{entail}) have an empty
symmetric difference 
$(p_{11} - p_{12}) + (p_{12} - p_{11})$, and so sentences
\exi{three}{a} and \exi{entail}{b} are equivalent, in the sense that they
denote the same set of worlds.  This relation is assessed computationally
in the possible worlds model.

\begin{examples} \label{entail}
\itema Amy knows that its tails.
\itemb \( \displaystyle
\neg \Diamond_{\text{Amy}} \neg \bullet^*t
)
\)
% \itemc \(\displaystyle \Box_{\text{Amy}}\Box_{\text{Bob}}(\Box_{\text{Amy}} \neg \bullet^* h + \Box_{\text{Amy}}\neg \bullet^* t)\) \hfill \mbox{} \(~~\cdot~~\Box_{\text{Amy}}\neg\Box_{\text{Bob}}\Box_{\text{Amy}}\neg \bullet^* t\)
\end{examples}
...
issue for further investigation is the mathematical characterizatiton
of doxastic KATs, e.g. axioms relating product and modality.
